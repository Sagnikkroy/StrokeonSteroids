...
if len(all_inkml) > 0:
    print(f'Dataset already extracted: {len(all_inkml)} files found. Skipping download.')
else:
    ...
    all_inkml = glob.glob(os.path.join(DATA_DIR, '**', '*.inkml'), recursive=True)
    # Also check parent in case folder name differs
    if not all_inkml:
...
def decode_batch(log_probs, n_strokes):
    return [ctc_greedy_decode(log_probs[i], n_strokes[i].item())
...
for epoch in range(start_epoch, args.epochs + 1):
    ...
    msg = (f'Epoch {epoch:3d}/{args.epochs} | '
           ...
           f'CER={val_cer:.4f} | {elapsed:.0f}s{saved}')
    log(msg)
    ...
    # Save history JSON every 10 epochs
    if epoch % 10 == 0:
